 ...
 *
 * V4.2.12 起如果开启了 task_enable_coroutine 则回调函数原型是: function (\Swoole\Server|\Swoole\Http\Server|\Swoole\WebSocket\Server $server, \Lee2son\Swoolaravel\Swoole\Server|\Lee2son\Swoolaravel\Swoole\Http\Server|\Lee2son\Swoolaravel\Swoole\WebSocket\Server\Task $task)
 *
 * 返回执行结果到 worker 进程：
 *  1.在 onTask 函数中 return 字符串，表示将此内容返回给 worker 进程。
 *    worker 进程中会触发 onFinish 函数，表示投递的 task 已完成，当然你也可以通过 \Lee2son\Swoolaravel\Swoole\Server|\Lee2son\Swoolaravel\Swoole\Http\Server|\Lee2son\Swoolaravel\Swoole\WebSocket\Server->finish() 来触发 onFinish 函数，而无需再 return
 ...
 * @see \Lee2son\Swoolaravel\Swoole\Server|\Lee2son\Swoolaravel\Swoole\Http\Server|\Lee2son\Swoolaravel\Swoole\WebSocket\Server\Task
 */
class Task
{
    const SWOOLE_EVENT_NAME = 'task';
